function Gauge(opt) {
  if(typeof opt.gaugeRadius === 'undefined')      {opt.gaugeRadius=100}
    ...
    if(typeof opt.edgeWidth === 'undefined')        {opt.edgeWidth=0.10}
    ...
    if(typeof opt.needleTickGap === 'undefined')    {opt.needleTickGap=0.00}
    //needleTailLength
    if(typeof opt.needleLengthNeg === 'undefined')  {opt.needleLengthNeg=0.12}
    //needleBaseWidth / 2
    ...
    if(typeof opt.pivotCol === 'undefined')         {opt.pivotCol = '#F14E49'}
    ...
    if(typeof opt.needleCol === 'undefined')        {opt.needleCol = '#F14E49'}
    ...
    counter=0
    var tickLabelText=[];
    // tickLabelText.push(opt.minVal)
    // tickLabelText.push(opt.maxVal)
    ...
    var pivotC = circleGroup.append("svg:circle")
            ...
            .style("stroke", "none");
    var innerPivotC = circleGroup.append("svg:circle")
            .attr("cx", originX)
            .attr("cy", originY)
    ...
    for (var index in opt.yellowZones)
    {
      drawBand(valueScale(opt.yellowZones[index].from) - valueScale(0), valueScale(opt.yellowZones[index].to)- valueScale(0), opt.yellowColor);
    }
    for (var index in opt.orangeZones)
    {
      drawBand(valueScale(opt.orangeZones[index].from)- valueScale(0), valueScale(opt.orangeZones[index].to)- valueScale(0), opt.orangeColor);
    }
    for (var index in opt.redZones)
    {
      drawBand(valueScale(opt.redZones[index].from)- valueScale(0), valueScale(opt.redZones[index].to)- valueScale(0), opt.redColor);
    }
   ...
   //      pathTickMin = tickCalcMin();
    ...
    needleCalc = function() {
            function pathCalc(d,i) {
                ...
                //x1 y1 = left
                //x3 y3 = right
                var y1 = originY + (opt.pivotRadius * Math.sin(nAngleRad - Math.PI / 2)),
                    y2 = originY + ((needlePathStart + needlePathLength) * Math.sin(nAngleRad)),
                    y3 = originY + (opt.pivotRadius * Math.sin(nAngleRad+ Math.PI / 2)),
                    x1 = originX - (opt.pivotRadius * Math.cos(nAngleRad) - Math.PI / 2),
                    x2 = originX + ((needlePathStart + needlePathLength) * Math.cos(nAngleRad)),
                    x3 = originX + (opt.pivotRadius * Math.cos(nAngleRad+ Math.PI / 2)),
                    lineData = [{"x": x1, "y": y1}, {"x": x2, "y": y2}, {"x": x3, "y": y3}];
    ...
    {
      //console.log(opt.tickLengthMaj)
      if (0 >= end - start) return;
      svg.append("svg:path")
        .style("fill", color)
        .style("stroke", "#E4E4E4")
        .attr("d", d3.arc()
            .innerRadius(opt.gaugeRadius * 0.50)
            .outerRadius(innerEdgeRadius)
            .startAngle(dToR(start))
            .endAngle(dToR(end))
            )
        .attr("transform", function() { return "translate(" + originX + ", " + originY + ") rotate(240)" });
