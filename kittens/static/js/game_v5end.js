...
var draw = function(e){
  ...
  console.log("Draw funcion, currentPlayer should be 0: " + currentPlayer);
  e.stopPropagation();
  var card = deck.pop();
  card = e.target;
...
var opp_draw = function() {
  var card = deck.pop()
  console.log("Draw funcion, currentPlayer should be 1: " + currentPlayer);
  players[currentPlayer].Hand.push(card)
  var type = card.getAttribute("type");
...
var move_center = function(e){
  ...
  remove_card(players[0].Hand, type);

}

// Opponents move it to the center
var opp_move_center = function(card) {
  ...
  card.removeEventListener("mouseleave", reset_position);
  card.removeEventListener("click", move_center);
  remove_card(players[1].Hand, type);
...
var favor = function(){
  ...
  if (currentPlayer == 0) {
    y = 400
    favor_card.addEventListener("mouseover", hover);
    favor_card.addEventListener("mouseleave", reset_position);
  }
  if (currentPlayer == 1) {
    y = 0
    favor_card.removeEventListener("mouseover", hover);
    favor_card.removeEventListener("mouseleave", reset_position);
  }
  arrange_cards(players[currentPlayer].Hand, y)
}
...
var init = function() {
  ...
  deck.push(explode);
  var explode2 = make_card("explode");
  explode2.addEventListener("click", draw);
  deck.push(explode2);
  var explode3 = make_card("explode");
  explode3.addEventListener("click", draw);
  deck.push(explode3);
  shuffle(deck);
